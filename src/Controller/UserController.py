...
class UserController:
    ...
    def set_preferences(self, title: str, location: str) -> bool:
        """
        Set user's job preferences
        Returns True if successful, False otherwise
        """
        if not self.user_id or not title or not location:
            return False
            
        return self.db.update_preferences(self.user_id, title, location)

    def get_preferences(self) -> Dict[str, str | None]:
        """
        ...
        return self.db.get_user_preferences(self.user_id)